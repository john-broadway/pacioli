...
⚠️ **A near-copy of the broker's ``test_citation_integrity.py``, and deliberately so.**
``pacioli-guard`` is a separate distribution with ZERO dependencies — it cannot import from the
broker, and giving it one for a test would trade a real architectural property for a little less
duplication. The broker's copy walks only the broker package, so without this file the guard's
prose went unchecked; an independent review pointed that out after 8 of its 13 findings landed in
``pacioli_guard/``.
...
#: Modules whose prose is checked. The whole package except the tests themselves — RECURSIVE,
#: because guard keeps real logic under `scoping/doctype/` (the consent-marker controller lives
#: there and carries the immutable-marker guard's own residual notes). A top-level-only glob
#: would have left exactly that file unchecked.
SOURCE_FILES = sorted(p for p in PACKAGE.rglob("*.py")
...
class TestNoSelfCitationCarriesALineNumber(unittest.TestCase):
    ...
    def test_the_guard_is_reading_real_data(self):
        for module in ("scope", "enforce", "act", "api"):
            self.assertIn(module, self.OWN_MODULES)
        self.assertGreater(len(SOURCE_FILES), 5)
        self.assertTrue(
            any(p.name == "pacioli_consent_marker.py" for p in SOURCE_FILES),
            "the recursive walk must reach the DocType controllers, or the module carrying the "
            "immutable-marker guard goes unchecked")
